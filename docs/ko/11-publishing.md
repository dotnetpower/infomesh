...
└── .github/workflows/
   ├── ci.yml              # CI: 린트, 포맷, mypy, 테스트, 빌드 (push/PR 시)
   ├── auto-release.yml    # main 변경 시 자동 버전 bump/tag/publish
   └── publish.yml         # GitHub Release 기반 수동 trusted PyPI publish
```
...
browser = ["playwright>=1.40"]           # JS 렌더링
cjk = ["jieba>=0.42"]                    # CJK 토큰화
all = ["infomesh[p2p,vector,llm,browser,cjk]"]  # 전체
...
`auto-release.yml`은 소스, 테스트, 패키징, 시드, 부트스트랩 데이터, 릴리스
workflow가 `main`에 push될 때 실행됩니다. lint, format, mypy, tests를 실행한 뒤
patch 버전을 올리고, `uv.lock`을 갱신하고, 릴리스 태그와 산출물을 만들고,
Trusted Publishing으로 PyPI에 게시합니다. 부분 실패 후 재시도해도 안전하도록
기존 GitHub Release는 건너뛰고 PyPI publish는 `skip-existing`을 사용합니다.

### 수동 릴리스 프로세스

```bash
# 1. 버전 업데이트
# infomesh/__init__.py와 pyproject.toml 편집 후 uv lock 실행
...
# 태그 vX.Y.Z 선택, 릴리스 노트 작성, "Publish release" 클릭
...
- [ ] 린트 오류 없음: `uv run ruff check infomesh/ tests/`
- [ ] 포맷 체크 통과: `uv run ruff format --check .`
- [ ] 타입 체크 통과: `uv run mypy infomesh/ --ignore-missing-imports`
- [ ] 지원 테스트 통과: `uv run pytest tests/ --ignore=tests/test_vector.py -x -q --tb=short`
- [ ] `pyproject.toml`과 `__init__.py` 버전 업데이트 및 `uv.lock` 갱신
- [ ] README.md 최신 상태
