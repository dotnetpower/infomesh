...
- LLM 스케줄러는 심야 시간대 결정을 `is_off_peak=true`로 표시합니다. 크레딧
  기록은 LLM 행동이 이 off-peak 플래그와 함께 기록될 때만 1.5배 배율을 적용합니다.
...
| `crawl_url()` 속도 제한 | 노드당 60 URL/시간, 도메인당 10개 대기열, depth 무제한 (0=무제한, 설정 가능). 같은 도메인만 추적. |

## 교차 노드 크레딧 동기화

같은 **GitHub 이메일**로 여러 기기(예: PC와 노트북)에서 InfoMesh를 실행하면, 각 기기는 별도의 로컬 크레딧 원장을 유지합니다. 크레딧 동기화 시스템은 이러한 노드들이 서로를 발견하고 모든 기기에 걸친 **통합** 크레딧 통계를 표시할 수 있게 합니다.

### 작동 방식

1. **피어 발견**: 연결 시 각 노드는 GitHub 이메일의 SHA-256 해시를 공유합니다 (개인정보 보호 — 평문 이메일은 절대 전송되지 않음).
2. **동일 소유자 매칭**: 두 피어가 같은 이메일 해시를 공유하면, 같은 소유자의 노드로 인식합니다.
3. **요약 교환**: 매칭된 피어들은 총 수입, 총 지출, 기여 점수, 티어가 포함된 서명된 크레딧 요약을 교환합니다.
4. **집계**: 각 노드는 로컬 통계와 수신된 피어 요약을 병합하여 통합 뷰를 계산합니다.
5. **주기적 동기화**: 요약은 5분마다 재교환되어 최신 상태를 유지합니다.

### 요구 사항

- `~/.infomesh/config.toml`의 `[node]` 섹션에 GitHub 이메일을 설정하세요:
  ```toml
  github_email = "you@example.com"
  ```
  또는 `git config user.email`로 설정하면 자동 감지됩니다.
- P2P 모드가 활성화되어 있고 노드들이 같은 네트워크에 연결되어 있어야 합니다.

### 개인정보 & 보안

| 항목 | 상세 |
|------|------|
| 이메일 프라이버시 | SHA-256 해시만 교환, 평문 전송 안 함 |
| 무결성 | 각 요약은 노드의 Ed25519 키로 서명됨 |
| 이중 계산 방지 | 요약은 peer_id로 키 지정; 각 피어는 정확히 하나만 기여 |
| 만료 정리 | 72시간 이상 된 피어 요약은 자동 삭제 |
| DoS 제한 | 소유자당 최대 20개 피어 요약 저장 |

### 통합 크레딧 확인

- **MCP `status` / `credit_balance` 도구**: 같은 소유자 노드가 동기화되면 `network` 하위 객체가 포함됩니다.
- **대시보드**: 크레딧 탭에서 로컬 및 네트워크 전체 잔액을 표시합니다.
- **텍스트 리포트** (`infomesh dashboard --tab credits`): 로컬 통계 아래에 네트워크 합계를 표시합니다.
