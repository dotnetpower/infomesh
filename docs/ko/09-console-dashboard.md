...
```
infomesh/dashboard/            # Textual 대시보드 패키지
├── __init__.py
├── app.py              # DashboardApp (메인 Textual Application)
├── bgm.py              # BGMPlayer (배경 음악 재생 — mpv/ffplay)
├── text_report.py      # Rich 기반 텍스트 리포트 (비대화형 폴백)
├── screens/
│   ├── __init__.py
│   ├── overview.py     # OverviewPane — 노드, 리소스, 활동, 라이브 로그
│   ├── crawl.py        # CrawlPane — 크롤 통계, 도메인, 라이브 로그
│   ├── search.py       # SearchPane — 입력과 검색 결과
│   ├── network.py      # NetworkPane — P2P, DHT, 피어, 대역폭
│   ├── credits.py      # CreditsPane — 잔액, 수입, 거래 기록
│   └── settings.py     # SettingsPane — 편집 가능한 설정 섹션
├── widgets/
│   ├── __init__.py
│   ├── sparkline.py    # SparklineChart (Unicode 블럭 미니 차트)
│   ├── bar_chart.py    # BarChart + BarItem (수평 막대 그래프)
│   ├── resource_bar.py # ResourceBar (CPU/RAM/Disk/Net 리소스 바)
│   └── live_log.py     # LiveLog (실시간 이벤트 로그, RichLog 기반)
...
대시보드 BGM 재생 시작 시 `mpv`가 없으면 InfoMesh가 먼저 비대화형 best-effort
방식으로 `mpv` 자동 설치를 시도합니다. 설치할 수 없거나 실패하면 `ffplay`로
폴백하고, 둘 다 없으면 BGM은 자동으로 비활성화됩니다 (오류 없음).
...
bgm_auto_install_mpv = true  # true = mpv 없을 때 best-effort 자동 설치
bgm_volume = 50           # 0–100
bgm_idle_stop = false     # true = 크롤링 유휴 시 BGM 자동 정지
```
...
크롤링 및 인덱싱과 CPU를 놓고 경쟁하여 음악이 끊길 수 있습니다. InfoMesh는
**mpv**를 우선 사용하고 gapless 루프와 오디오 버퍼를 켜며, 가능하면 `mpv`를
자동 설치합니다. 그래도 끊김이 있으면:

1. `bgm_auto_install_mpv = true`인지 확인하거나, 자동 설치가 불가능한 환경이면
  `mpv`를 수동 설치
2. `m` 키를 눌러 BGM 비활성화
...
TUI 외에도, InfoMesh는 관리 API가 실행 중일 때 브라우저에서
`http://localhost:8080/dashboard`로 접근 가능한 **웹 기반 대시보드**를 제공합니다.

### 기능

- **5개 탭 페이지**: 개요, 검색 분석, 크롤 상태, 네트워크, 크레딧
- **자동 갱신**: 5초마다 API 엔드포인트 폴링
- **무의존성**: 순수 HTML/JS — 빌드 단계, npm 불필요
- **다크 테마**: GitHub 스타일 다크 모드
- **로컬호스트 전용**: 관리 API와 동일한 보안 미들웨어

### 대시보드 접근

```bash
# 노드 시작 (관리 API가 자동으로 시작)
infomesh start
...
| 탭 | 표시 메트릭 |
|-----|---------------|
| **개요** | 노드 상태, 인덱스 크기, 피어, 크레딧, 검색, 크롤 |
| **검색** | 총 검색 수, 평균 지연 시간 (컬러 바), 페치 비율 |
| **크롤** | 총 크롤 수, 인덱싱된 문서, 평균 문서 크기 |
| **네트워크** | 연결된 피어, DHT 모드, 피어 ID |
| **크레딧** | 잔액, 티어, 총 획득/지출 |
