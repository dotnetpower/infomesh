...
P2P를 사용할 수 있으면 MCP 시작 시 기존 로컬 인덱스를 분산 DHT에
다시 발행하고, `crawl_url`은 새로 인덱싱한 페이지를 즉시 발행합니다.
`local_only=true`를 설정하면 P2P 탐색을 건너뛰고 로컬 FTS 인덱스만
검색합니다.
...
InfoMesh는 MCP 툴 사용 패턴에서 암묵적 품질 시그널을 추적하여
시간이 지남에 따라 검색 랭킹을 개선합니다. 모든 데이터는 로컬 전용이며
개인정보를 보호합니다 (쿼리 텍스트는 SHA-256 해시로 저장).

| 시그널 | 트리거 | 효과 |
|--------|--------|------|
| `fetched` | `web_search` → `fetch_page` 변환 | URL 부스트 +1.0 |
| `skipped` | 결과가 반환되었지만 페치하지 않음 | URL 감소 -0.3 |
| `reformulated` | 60초 내 동일 쿼리 해시 | 낮은 품질 표시 |
| `cited` | `fact_check` 증거로 URL 사용 | URL 부스트 +2.0 |

설정 (`config.toml`):
```toml
[search]
feedback_tracking = true   # 기본값: true
```

CLI 검사:
```bash
infomesh feedback stats          # 시그널 수 + 상위 URL
infomesh feedback top-urls -n 20 # 최고 품질 URL
```

### CJK 언어 지원

중국어, 일본어, 한국어 쿼리는 자동 감지되어 FTS5 호환성을 위한
문자 바이그램 확장으로 전처리됩니다.

- 유니코드 문자 비율 분석을 통한 자동 감지
- CJK 문자에 대한 바이그램/트라이그램 생성
- CJK 토큰과 함께 라틴 단어 보존
- 선택 사항: `pip install 'infomesh[cjk]'`로 jieba (중국어 분할) 설치

### 쿼리 확장

초기 검색 결과가 부족할 때 (요청보다 적을 때), InfoMesh는 내장된
기술 용어 사전의 동의어로 쿼리를 자동 확장합니다
(error↔exception, api↔endpoint 등 20개 이상의 매핑).
