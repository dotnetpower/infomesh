...
- **mpv** 또는 **ffplay** — 선택 사항, 대시보드 배경음악에만 필요;
  InfoMesh는 첫 BGM 재생 시 비대화형 best-effort 방식으로 `mpv` 자동 설치를 시도
...
보통 CPU 경합이 원인입니다. InfoMesh는 장시간 실행되는 크롤 worker를
설정된 낮은 CPU/I/O 우선순위로 실행하고, 대시보드와 BGM은 상호작용용
control 프로세스에 남겨 둡니다. 저사양 장치에서 여전히 끊기면 다음을
추가로 적용하세요:

1. **BGM은 v0.1.2부터 기본 꺼짐**입니다. `m` 키로 수동 활성화하거나
   `~/.infomesh/config.toml`에서 `bgm_auto_start = true` 설정
2. `bgm_auto_install_mpv = true`를 유지하거나, 자동 설치가 불가능한 환경이면
  `mpv`를 수동 설치 — InfoMesh는 gapless 루프와 버퍼 재생을 위해 `mpv`를
  우선 사용합니다
3. 대시보드 새로고침 간격 줄이기:
   ```toml
   [dashboard]
...
BGM idle-stop은 간헐적인 크롤링 중 stop/start 끊김을 피하기 위해 기본적으로
꺼져 있습니다. BGM을 크롤링 활동 신호로 사용하려면 활성화하세요:
...
InfoMesh는 대시보드 BGM 시작 시 **mpv** 자동 설치를 시도합니다. 설치는
비대화형으로만 진행되므로 root/passwordless sudo/Homebrew가 없으면 **mpv** 또는
**ffplay**를 수동 설치하세요:
```bash
# Debian/Ubuntu:
...
자동 설치가 실패하고 두 플레이어 모두 없으면 BGM은 자동으로 비활성화됩니다
(오류 없음). 자동 설치 시도를 끄려면 `bgm_auto_install_mpv = false`를 설정하세요.
...
### InfoMesh는 중국어/일본어/한국어 검색을 지원하나요?

네. InfoMesh는 쿼리에서 CJK 문자를 자동 감지하고 FTS5 호환성을 위한
문자 바이그램 확장을 적용합니다. 더 나은 중국어 분할을 위해 선택적
CJK 패키지를 설치하세요:

```bash
pip install 'infomesh[cjk]'  # 중국어용 jieba 설치
```

CJK 중심 인덱스의 경우 트라이그램 토크나이저를 설정하세요:
```toml
[index]
...
InfoMesh는 MCP 툴 사용에서 LLM 네이티브 품질 시그널을 추적합니다:
- **페치 시그널**: LLM이 `fetch_page`를 통해 검색 결과를 페치하면 해당 URL의 품질 점수가 올라갑니다
- **스킵 시그널**: 반환되었지만 페치되지 않은 결과는 약간의 감점을 받습니다
- **인용 시그널**: `fact_check`에서 사용된 URL은 강한 부스트를 받습니다

모든 시그널은 SHA-256 해시로 로컬에 저장됩니다 (평문 쿼리 없음).
`[search] feedback_tracking = false`로 비활성화할 수 있습니다.

### 웹 대시보드에 어떻게 접근하나요?

노드가 실행 중일 때 브라우저에서 `http://localhost:8080/dashboard`를
열면 됩니다. 개요, 검색 분석, 크롤 상태, 네트워크, 크레딧의 5개 탭을
보여줍니다. 5초마다 자동 갱신됩니다.

### RSS 피드를 가져올 수 있나요?

네. InfoMesh는 우선순위 기반 폴링을 갖춘 RSS/Atom 피드 모니터를 포함합니다:

```bash
infomesh feeds import subscriptions.opml   # OPML에서 가져오기
infomesh feeds list                        # 모니터링 중인 피드 보기
```

### 프로덕션 클러스터를 어떻게 부트스트랩하나요?

포함된 Terraform 설정을 사용하세요:
...
이것은 Azure에 지리적으로 분산된 3개의 부트스트랩 노드를 배포합니다
